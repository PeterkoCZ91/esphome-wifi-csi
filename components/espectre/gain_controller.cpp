...
void GainController::process_packet(const wifi_csi_info_t* info) {
  ...
  if (packet_count_ < CALIBRATION_PACKETS) {
    ...
    if (mode_ == GainLockMode::DISABLED) {
      ...
      // Recommended operating band (ESPectre tuning guide): AGC 30-60.
      // AGC > 60 indicates weak signal / low SNR — detection may degrade.
      if (agc_gain_locked_ > 60) {
        ESP_LOGW(TAG, "AGC=%d above recommended max (60) — weak signal, expect degraded FP rate",
                 agc_gain_locked_);
      }
    }
