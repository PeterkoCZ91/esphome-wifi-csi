 ...
 *   - espnow: ESP-NOW broadcast frames (legacy PHY, ~8 pkt/s CSI on C5/C6)
...
enum class TrafficGeneratorMode {
  ...
  ESPNOW,  // ESP-NOW broadcast frames (legacy PHY, limited CSI on C5/C6)
 ...
 * 
 * Generates continuous WiFi traffic using UDP/DNS queries, ICMP ping, or ESP-NOW
 ...
 * ESP-NOW mode: broadcast action frames (legacy PHY — limited CSI on C5/C6 HE hardware)
...
class TrafficGeneratorManager {
 public:
  ...
  /**
   * Pre-initialize ESP-NOW before CSI is enabled.
   * Must be called before csi_manager_.enable() when using ESPNOW mode.
   * On ESP32-C5, calling esp_now_init() after esp_wifi_set_csi() disables CSI.
   * No-op for DNS/PING modes.
   */
  bool pre_init();
 ...
 private:
  // FreeRTOS task function (static wrapper) - DNS and UDP modes
  static void dns_traffic_task_(void* arg);
  static void espnow_traffic_task_(void* arg);
