...
void ESpectreComponent::setup() {
  ...
  if (this->has_peer_mac_) {
    this->csi_manager_.set_peer_mac_filter(this->peer_mac_);
    if (this->has_peer_mac_accept_broadcast_) {
      this->csi_manager_.set_peer_mac_filter_also_broadcast(true);
    }
    if (this->has_peer_mac_accept_ap_bssid_) {
      this->csi_manager_.set_peer_mac_filter_also_ap_bssid(true);
    }
    for (auto &mac : this->extra_peer_macs_) {
...
void ESpectreComponent::on_wifi_connected_() {
  ...
  if (this->traffic_generator_mode_ == TrafficGeneratorMode::ESPNOW) {
