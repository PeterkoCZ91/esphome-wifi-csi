 ...
 *
 * Extracts 17 non-redundant features from CSI data for ML-based motion
 * detection. Port of micro-espectre/src/features.py to C++.
 *
 * Features 0-11 are computed from the turbulence buffer (75 samples).
 * Features 12-14 are multi-domain signals from BaseDetector.
 * Features 15-16 are Uni-Fi (arXiv 2601.10980) channel statistics.
 *
 * Features (in order):
 ...
 * 13. ratio_turbulence - SA-WiSense adjacent subcarrier amplitude ratios
 * 14. breathing_score  - Bandpass-filtered amplitude energy (0.08-0.6 Hz)
 * 15. dser             - Uni-Fi Dynamic-to-Static Energy Ratio (log)
 * 16. plcr             - Uni-Fi Path-Length Change Rate proxy
 *
...
// Features 0-11: turbulence statistics
// Features 12-14: phase_turbulence, ratio_turbulence, breathing_score
// Features 15-16: dser, plcr (Uni-Fi)
...
/**
 * Extract all 17 ML features from turbulence buffer and multi-domain signals.
 *
 * Features 0-11 are computed from the turbulence buffer (typically 75 samples).
 * Features 12-14 are multi-domain signals from BaseDetector.
 * Features 15-16 are Uni-Fi DSER/PLCR statistics (last packet).
 *
 * @param turb_buffer Turbulence buffer
 * @param turb_count Number of valid values in turbulence buffer
 * @param phase_turbulence Std of inter-subcarrier phase diffs (last packet)
 * @param ratio_turbulence SA-WiSense adjacent subcarrier amplitude ratio std
 * @param breathing_score Bandpass-filtered amplitude energy (0.08-0.6 Hz)
 * @param dser Dynamic-to-Static Energy Ratio (log, Uni-Fi eq. 3)
 * @param plcr Path-Length Change Rate proxy (Uni-Fi eq. 4)
 * @param features_out Output array for 17 features (must be pre-allocated)
 */
inline void extract_ml_features(const float* turb_buffer, uint16_t turb_count,
                                float phase_turbulence,
                                float ratio_turbulence,
    ...
    features_out[12] = phase_turbulence;  // 12: std of inter-subcarrier phase diffs
    features_out[13] = ratio_turbulence;  // 13: SA-WiSense adjacent amplitude ratio std
    features_out[14] = breathing_score;   // 14: bandpass amplitude energy (0.08-0.6 Hz)
    features_out[15] = dser;              // 15: Uni-Fi Dynamic-to-Static Energy Ratio
    features_out[16] = plcr;              // 16: Uni-Fi Path-Length Change Rate proxy
}
