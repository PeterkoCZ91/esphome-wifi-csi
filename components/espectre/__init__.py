...
# Hysteresis (fork addition)
CONF_HYSTERESIS_FACTOR = "hysteresis_factor"

# Auto-calibration (fork addition)
CONF_AUTO_CALIBRATION_MINUTES = "auto_calibration_minutes"
...
# Pairwise/mesh sensing: MAC address of peer transmitter node
...
CONF_BREATHING_SENSOR = "breathing_sensor"
CONF_PHASE_TURBULENCE_SENSOR = "phase_turbulence_sensor"
CONF_PRESENCE_SENSOR = "presence_sensor"
CONF_DSER_SENSOR = "dser_sensor"
CONF_PLCR_SENSOR = "plcr_sensor"
CONF_BREATHING_RATE_SENSOR = "breathing_rate_sensor"
...
CONFIG_SCHEMA = cv.Schema({
    ...
    #   - auto (default): P95 x 1.1 - balanced sensitivity/false positives
    ...
    # Traffic generator mode: dns (default), ping (ICMP), espnow (pairwise, legacy PHY),
    ...
    # Hysteresis factor for asymmetric IDLE<->MOTION transitions (fork addition)
    ...
    # Auto-calibration: trigger once when environment is quiet for N minutes (0 = disabled)
    # ESP monitors mean_turbulence + variance -- calibrates automatically on first quiet period
    cv.Optional(CONF_AUTO_CALIBRATION_MINUTES, default=10): cv.int_range(min=0, max=60),

    # Pairwise/mesh sensing: MAC of peer transmitter — enables promiscuous mode + MAC filter
    ...
    # Extended sensors (fork additions — optional, disabled by default)
    cv.Optional(CONF_BREATHING_SENSOR): sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=4,
        state_class=STATE_CLASS_MEASUREMENT,
        icon="mdi:lungs",
    ),
    cv.Optional(CONF_PHASE_TURBULENCE_SENSOR): sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=4,
        state_class=STATE_CLASS_MEASUREMENT,
        icon="mdi:sine-wave",
    ),
    cv.Optional(CONF_PRESENCE_SENSOR): binary_sensor.binary_sensor_schema(
        device_class="occupancy",
    ),
    cv.Optional(CONF_DSER_SENSOR): sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=3,
        state_class=STATE_CLASS_MEASUREMENT,
        icon="mdi:chart-bell-curve",
    ),
    cv.Optional(CONF_PLCR_SENSOR): sensor.sensor_schema(
        unit_of_measurement=UNIT_EMPTY,
        accuracy_decimals=4,
        state_class=STATE_CLASS_MEASUREMENT,
        icon="mdi:speedometer",
    ),
    cv.Optional(CONF_BREATHING_RATE_SENSOR): sensor.sensor_schema(
...
async def to_code(config):
    ...
    # Configure hysteresis (fork addition)
    cg.add(var.set_hysteresis_factor(config[CONF_HYSTERESIS_FACTOR]))

    # Configure auto-calibration (fork addition)
    auto_cal_minutes = config[CONF_AUTO_CALIBRATION_MINUTES]
    cg.add(var.set_auto_calibration_enabled(auto_cal_minutes > 0))
    if auto_cal_minutes > 0:
        cg.add(var.set_auto_calibration_minutes(auto_cal_minutes))

    # Configure peer MAC filter for pairwise/mesh sensing
    if CONF_PEER_MAC in config:
        cg.add(var.set_peer_mac(str(config[CONF_PEER_MAC])))
    if "peer_macs" in config:
        for mac in config["peer_macs"]:
            cg.add(var.add_peer_mac(str(mac)))
    ...
    # Register extended sensors (optional, fork additions)
    if CONF_BREATHING_SENSOR in config:
        sens = await sensor.new_sensor(config[CONF_BREATHING_SENSOR])
        cg.add(var.set_breathing_sensor(sens))

    if CONF_PHASE_TURBULENCE_SENSOR in config:
        sens = await sensor.new_sensor(config[CONF_PHASE_TURBULENCE_SENSOR])
        cg.add(var.set_phase_turbulence_sensor(sens))

    if CONF_PRESENCE_SENSOR in config:
        sens = await binary_sensor.new_binary_sensor(config[CONF_PRESENCE_SENSOR])
        cg.add(var.set_presence_binary_sensor(sens))

    if CONF_DSER_SENSOR in config:
        sens = await sensor.new_sensor(config[CONF_DSER_SENSOR])
        cg.add(var.set_dser_sensor(sens))

    if CONF_PLCR_SENSOR in config:
        sens = await sensor.new_sensor(config[CONF_PLCR_SENSOR])
        cg.add(var.set_plcr_sensor(sens))

    if CONF_BREATHING_RATE_SENSOR in config:
        sens = await sensor.new_sensor(config[CONF_BREATHING_RATE_SENSOR])
